...
index.test= index.train2 & index.notna#(is.na(dgComp[,4]) ==T)
...
p.fac[1:265] = 0
fitf = glmnet(X.train,Y.train,family='binomial',penalty.factor=p.fac,intercept=F)
Y.test.logistic1 = predict(fitf,newx=X.test,type = "response", s=c(fitf$lambda[90]))
Y.test.class = matrix(NA,length(Y.test.logistic1[,1]),2)
Y.test.class[Y.test.logistic1[,1]<0.5,1] = -1
Y.test.class[Y.test.logistic1[,1]>0.5,1] = 1
finalData = data.frame(Y.test.class[,1],Y.test.class[,2],Y.test)
write.csv(finalData, file = 'interceptTest.csv', row.names=F)
...
save.image(file = '3MRanking.RData')
